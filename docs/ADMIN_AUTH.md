...
Without the callback URL, invite links will 404. Without the update-password URL, reset links may be rejected. Any server redirect (e.g. http→https) can drop the `#...` fragment and break the "Set new password" form.
...
4. **Link opens but "Set new password" form doesn't show** — Supabase normally redirects with the session in the URL hash. If your host or CDN does an extra redirect (e.g. HTTP→HTTPS), the hash can be lost. **Fix:** In **Authentication → Email Templates**, for **Reset Password** and **Invite user**, set the main link to use the token in the query: `{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=recovery&redirect_to=/admin/update-password` (use `type=invite` for Invite user). The app's `/auth/callback` will verify the token and redirect to the set-password page.
