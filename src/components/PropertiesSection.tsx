  ...
  return (
    <section id="residences" className="bg-luxury-dark py-24 lg:py-32">
      ...
      <div className="text-center px-6 mb-16 reveal-on-scroll">
        ...
        <h2 className="font-serif-luxury text-white leading-tight mb-4" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 300 }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[55%_45%] min-h-[560px] gap-px mb-px reveal-on-scroll">
        ...
        <div className="bg-luxury-stone px-10 py-16 lg:px-16 flex flex-col justify-center">
          ...
          <h3 className="font-serif-luxury text-white mb-3 leading-tight" style={{ fontSize: '2.4rem', fontWeight: 300 }}>
            Celestia<br /><em className="italic text-gold">Townhouses</em>
          </h3>
          <p className="text-luxury-mist text-sm leading-relaxed mb-6">
            Sophisticated 3-bedroom residences perched on tiered terraces with unobstructed mountain panoramas. Every room is designed around the view — and the view never disappoints.
          </p>
          <div className="flex flex-wrap gap-2 mb-6">
            {['Floor-to-ceiling glass', 'Rooftop infinity pool', 'Smart-home', 'Private terrace', 'Mountain views'].map((t) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-px">
        ...
        <div className="relative overflow-hidden min-h-[460px] group cursor-pointer">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-10">
            <span className="font-serif-luxury text-gold/10 leading-none block mb-[-0.3rem]" style={{ fontSize: '3rem', fontWeight: 300 }}>02</span>
            <span className="eyebrow-text block mb-2">Private Chalets</span>
            <h3 className="font-serif-luxury text-white mb-2 leading-tight" style={{ fontSize: '2rem', fontWeight: 300 }}>Celestia<br /><em className="italic text-gold">Chalets</em></h3>
            <p className="text-luxury-mist text-xs leading-relaxed mb-4">Built into the mountain face. Bespoke seclusion, river access, natural stone finishes, sunken fire pits.</p>
            <div className="flex flex-wrap gap-2 mb-4">
              {['Natural stone', 'Boat mooring', 'Fire pits'].map((t) => (
                <span key={t} className="text-[0.58rem] tracking-wide uppercase px-3 py-1 border border-gold/30 text-gold" style={{ background: 'rgba(201,168,76,0.08)' }}>{t}</span>
              ))}
            </div>
            <a href="https://www.brownstoneltd.com/celestia/chalets" target="_blank" rel="nofollow" className="inline-flex items-center gap-2 text-[0.65rem] tracking-[0.18em] uppercase text-gold hover:gap-4 transition-all duration-300">Explore Chalets <span>→</span></a>
          </div>
        </div>
        {/* Lakehouse */}
        <div className="relative overflow-hidden min-h-[460px] group cursor-pointer">
          ...
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(8,8,8,0.92) 0%, rgba(8,8,8,0.2) 60%, transparent 100%)' }} />
          <div className="absolute bottom-0 left-0 right-0 p-8 lg:p-10">
            <span className="font-serif-luxury text-gold/10 leading-none block mb-[-0.3rem]" style={{ fontSize: '3rem', fontWeight: 300 }}>03</span>
            <span className="eyebrow-text block mb-2">The Social Core</span>
            <h3 className="font-serif-luxury text-white mb-2 leading-tight" style={{ fontSize: '2rem', fontWeight: 300 }}>The<br /><em className="italic text-gold">Lakehouse</em></h3>
            <p className="text-luxury-mist text-xs leading-relaxed mb-4">Elite gym, thermal spa, executive deep-work hub — the communal heartbeat of Celestia, overlooking the Volta.</p>
            <div className="flex flex-wrap gap-2 mb-4">
              {['Thermal spa', 'Elite gym', 'Work hub'].map((t) => (
                <span key={t} className="text-[0.58rem] tracking-wide uppercase px-3 py-1 border border-gold/30 text-gold" style={{ background: 'rgba(201,168,76,0.08)' }}>{t}</span>
              ))}
            </div>
            <a href="https://www.brownstoneltd.com/celestia/lakehouse" target="_blank" rel="nofollow" className="inline-flex items-center gap-2 text-[0.65rem] tracking-[0.18em] uppercase text-gold hover:gap-4 transition-all duration-300">Explore Lakehouse <span>→</span></a>
          </div>
        </div>
      </div>
    </section>
