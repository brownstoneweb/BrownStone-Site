      ...
      <div className="max-w-[1100px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-end mb-16">
          <div className="reveal-on-scroll">
            <span className="eyebrow-text block mb-3">What&apos;s Built In</span>
            <h2 className="font-serif-luxury text-white leading-tight" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.2rem)', fontWeight: 300 }}>
              Technology That <em className="italic text-gold">Thinks</em>.<br />Spaces That Breathe.
            </h2>
          </div>
          <div className="reveal-on-scroll reveal-delay-2">
            ...
            <a href="https://www.brownstoneltd.com/celestia" target="_blank" rel="nofollow" className="inline-block border border-white/30 text-white px-8 py-3 text-[0.65rem] tracking-[0.18em] uppercase hover:border-gold hover:text-gold transition-all duration-300 whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[2px]">
          {features.map((f, i) => (
            <div key={f.title} className={`bg-luxury-stone p-10 relative overflow-hidden group transition-colors duration-300 hover:bg-luxury-charcoal reveal-on-scroll ${i % 3 === 1 ? 'reveal-delay-1' : i % 3 === 2 ? 'reveal-delay-2' : ''}`}>
              ...
              <h3 className="font-serif-luxury text-gold mb-3" style={{ fontSize: '1.15rem', fontWeight: 400 }}>{f.title}</h3>
              <p className="text-luxury-mist text-[0.82rem] leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
